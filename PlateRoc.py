...
def detectLicensePlate(img):
  rsp_json = {'Vehicle_Detected': 0, 'Plates':[]}
  Icars, Lcars = vehicle_detection(img)
  print('# vehicle detected: {}'.format(len(Icars)))
  # for each detected car in test image
  rsp_json['Vehicle_Detected'] = len(Icars)
  for Icar, Lcar in zip(Icars, Lcars):    
    # draw car bounding box on test image
    draw_label(img,Lcar,color=(0,255,255),thickness=3)
    ...
    for Llp, Ilp in zip(Llps, Ilps):
      ...
      draw_losangle(img,ptspx,color=(0,0,255),thickness=3)
      # Recognize characters
      lp_str = lp_ocr(Ilp)
      rsp_json['Plates'].append(lp_str)
      # write text on test image
      llp = Label(0,tl=pts.min(1),br=pts.max(1))
      write2img(img,llp,lp_str)
...
if  __name__ == '__main__':
  ...
  img, jsn = detectLicensePlate(test_img)
  print(jsn)
